...
unsigned short int tuneEnvelopeDelta[NOTES];
...
void tuneAddNote( int noteNumber,unsigned char  volume, int beat, int voice )
{
   ...
   while( tuneNextListIndex < tuneNoteCount && tuneListBeat[ tuneNextListIndex ] <= tuneNextBeat )
   {
     ...
     logTuneProgress();
      ...
      startNote (voice, 
                  ...
                  tuneVoice[ voice ], 
                  tuneEnvelopeDelta[voice], tuneEnvelope[voice], 7, tuneVibratoPercent[voice]);
...
void tuneSetEnvelopeDelta (unsigned short int which_generator, unsigned short int envelopeDelta)
{
  if ((which_generator >= 0) && (which_generator < NOTES))
    tuneEnvelopeDelta[which_generator] = envelopeDelta;
}
